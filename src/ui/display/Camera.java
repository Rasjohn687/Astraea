...
{	
	...
	public static final float ZOOM_SECRET_MAX = .1f;
	...
	public static final float SCROLL_RATE_KEY_ACCELERATION = 5f;
	...
	public static float getScreenCenterX()	{	return Main.getScreenWidth() / Camera.getZoom() / 2 - Camera.getX(); }
	public static float getScreenCenterY()	{	return Main.getScreenHeight() / Camera.getZoom() / 2 - Camera.getY(); }
	...
	public static void keyboard(GameContainer gc)
	{
		...
		{
			ySpeed = Math.min(0, ySpeed - SCROLL_RATE_KEY_ACCELERATION / getZoomScrollModifier());
		...
		{
			ySpeed = Math.max(0, ySpeed + SCROLL_RATE_KEY_ACCELERATION / getZoomScrollModifier());
			if(ySpeed >= SCROLL_RATE_KEY_MAX / getZoomScrollModifier())
			{
				ySpeed = SCROLL_RATE_KEY_MAX / getZoomScrollModifier();
			}
		...
		{
			xSpeed = Math.min(0, xSpeed - SCROLL_RATE_KEY_ACCELERATION / getZoomScrollModifier());
			xSpeed -= SCROLL_RATE_KEY_ACCELERATION;
		...
		{
			xSpeed = Math.max(0, xSpeed + SCROLL_RATE_KEY_ACCELERATION / getZoomScrollModifier());
