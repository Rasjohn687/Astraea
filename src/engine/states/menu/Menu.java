...
public class Menu extends BasicGameState
{
	...
	private static engine.states.Game g;

	private MenuBG background;

	private static final String[] blklst = {"neutral"};
	...
	public void init(GameContainer gc, StateBasedGame sbg) throws SlickException {
		...
		run = new Button<Integer>(Main.getScreenWidth() * .025f, Main.getScreenHeight() * .90f,  Main.getScreenWidth() * .235f, Main.getScreenHeight() * .060f, 1);
	...
	private void initButtons() {
		...
		float x2 = Main.getScreenWidth() * .155f;
		float y1 = Main.getScreenHeight() * .500f;
		float y2 = Main.getScreenHeight() * .540f;
		float w = Main.getScreenWidth() * .105f;
...
//					System.out.println("Printing file in class SelectionBattle: " + f);
		...
		List<Class<?extends Player>> getPlayer(File f) 
		{
			...
			for(File a : f.listFiles()) {
				...
				try {
					...
					Class<?> cs = ClassLoader.getSystemClassLoader().loadClass(a.getPath().substring(4, a.getPath().length() - 6).replace("\\", "."));
					if(Player.class.isAssignableFrom(cs) && !Modifier.isAbstract(cs.getModifiers())) {
	...
	public static void addPlayer(Class <? extends Player> clazz) {	
		try {
			...
			//Player t = (Player)(clazz.getConstructor(new Class<?>[] {})).newInstance(players.size(), g);
			Player t = clazz.getConstructor().newInstance();
...
//		}
	...
	public void render(GameContainer gc, StateBasedGame sbg, Graphics g) throws SlickException 
	{
		...
		// Upper Box
		g.setColor(new Color(5, 5, 5, 180));
		g.fillRoundRect(Main.getScreenWidth() * .01f, Main.getScreenHeight() * .02f, Main.getScreenWidth() * .27f, Main.getScreenHeight() * .18f, 25);

		g.setLineWidth(9);
		g.setColor(new Color(0, 0, 0, 255));
		g.drawRoundRect(Main.getScreenWidth() * .01f, Main.getScreenHeight() * .02f, Main.getScreenWidth() * .27f, Main.getScreenHeight() * .18f, 25);

		g.setLineWidth(5);
		g.setColor(new Color(100, 60, 200, 100));
		g.drawRoundRect(Main.getScreenWidth() * .01f, Main.getScreenHeight() * .02f, Main.getScreenWidth() * .27f, Main.getScreenHeight() * .18f, 25);

	// Lower Box
		g.setColor(new Color(5, 5, 5, 180));
		g.fillRoundRect(Main.getScreenWidth() * .01f, Main.getScreenHeight() * .48f, Main.getScreenWidth() * .27f, Main.getScreenHeight() * .50f, 25);

		g.setLineWidth(9);
		g.setColor(new Color(0, 0, 0, 255));
		g.drawRoundRect(Main.getScreenWidth() * .01f, Main.getScreenHeight() * .48f, Main.getScreenWidth() * .27f, Main.getScreenHeight() * .50f, 25);

		g.setLineWidth(5);
		g.setColor(new Color(100, 60, 200, 100));
		g.drawRoundRect(Main.getScreenWidth() * .01f, Main.getScreenHeight() * .48f, Main.getScreenWidth() * .27f, Main.getScreenHeight() * .50f, 25);

		float x = Main.getScreenWidth() * -.005f;
		float y = Main.getScreenHeight() * .025f;
		float w = Main.getScreenWidth() * .295f;
		float h = w / 1500 * 471;
		...
		Utility.drawStringRightTop(g, Fonts.mediumFont, "Astraeus Version " + Values.VERSION, Main.getScreenWidth() - 15, 15);
		Utility.drawStringRightTop(g, Fonts.mediumFont, "Released " + Values.RELEASE_DATE, Main.getScreenWidth() - 15, 35);
	...
	{
		background.keyPressed(gc, key, c);
	}
